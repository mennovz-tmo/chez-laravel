...
@section('content')
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <h1 class="display-5 mb-4 section-title">Recept bewerken</h1>
            <form action="{{ route('recipe.edit', ['recipe' => $current_data['id']]) }}" method="post" enctype="multipart/form-data" class="card p-4 shadow-sm card-body-container">
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Naam</label>
                    <input required type="text" class="form-control" id="name" name="name"
                        value="{{ old('name', $current_data['name']) }}" class="form-input">
                </div>
                <div class="mb-3">
                    ...
                    <textarea required class="form-control" name="description_short" id="description_short" rows="3"
                        class="form-input">{{ old('description_short', $current_data['description_short']) }}</textarea>
                </div>
                <div class="mb-3">
                    ...
                    <input required type="text" class="form-control" id="allergens" name="allergens"
                        value="{{ old('allergens', $current_data['allergens']) }}" class="form-input">
                </div>
                <div class="mb-3">
                    <label for="price" class="form-label">Prijs</label>
                    <input required type="number" class="form-control" id="price" name="price" step="0.01" min="0.01"
                        value="{{ old('price', $current_data['price']) }}" class="form-input">
                </div>
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" id="picture" name="picture"
                        accept=".png,.jpg,.webp,.avif,.jpeg,image/png,image/jpeg,image/webp" class="form-input">
                </div>
