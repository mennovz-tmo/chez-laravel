...
@section('content')
    <h1 class="display-4 mb-4 section-title">Statistieken</h1>
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <x-chart id="reservations-per-day" title="Reserveringen en gasten per dag (laatste 14 dagen)"
                :labels="$reservationLabels" :datasets="[
                    ['label' => 'Reserveringen', 'data' => $reservationsPerDay],
                    ['label' => 'Gasten', 'data' => $guestsPerDay],
                ]" />
        </div>
        <div class="col-lg-6">
            <x-chart id="arrivals-by-hour" title="Aankomsten per uur" :labels="$arrivalLabels" :datasets="[
                ['label' => 'Reserveringen', 'data' => $arrivalCounts],
            ]" />
        </div>
        <div class="col-lg-6">
            <x-chart id="weekly-hours" title="Open uren per weekdag" :labels="$weeklyHoursLabels" :datasets="[
                ['label' => 'Uren open', 'data' => $weeklyHoursData],
            ]" />
        </div>
        <div class="col-lg-6">
            <x-chart id="specials-open-closed" type="doughnut" title="Bijzondere dagen: open vs gesloten"
                :labels="['Open', 'Gesloten']" :datasets="[
                    ['label' => 'Dagen', 'data' => [$specialOpen, $specialClosed]],
                ]" />
        </div>
        <div class="col-lg-6">
            <x-chart id="recipe-prices" type="bar" title="Prijzen per recept (top 12)" :labels="$recipePriceLabels"
                :datasets="[['label' => 'Prijs (€)', 'data' => $recipePriceData]]" />
        </div>
        <div class="col-lg-6">
            <x-chart id="allergens" type="doughnut" title="Recepten per allergeen" :labels="$allergenLabels" :datasets="[
