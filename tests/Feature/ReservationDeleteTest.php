...
test('guest delete request sends verification email and keeps reservation', function () {
    ...
    $response = $this->post("/reservation/{$reservation->number}/delete");
...
test('guest confirms deletion with valid token from email', function () {
    ...
    $reservation = deletableReservation();
    $this->post("/reservation/{$reservation->number}/delete");
    ...
    $response = $this->get("/reservation/{$reservation->number}/delete/{$plainToken}");
...
test('guest cannot delete with invalid token', function () {
    ...
    $response = $this->get("/reservation/{$reservation->number}/delete/invalid-token");
...
test('staff user deletes immediately without email', function () {
    ...
    $response = $this->actingAs($user)->post("/reservation/{$reservation->number}/delete");
...
test('reservation cannot be deleted through a plain GET request', function () {
    $user = User::factory()->state(['role' => 'staff'])->create();
    $reservation = deletableReservation();

    $this->actingAs($user)
        ->get("/reservation/{$reservation->number}/delete")
        ->assertStatus(405);
